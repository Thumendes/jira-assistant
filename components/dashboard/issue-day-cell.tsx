...
function IssueLogsModalContent(props: {
  ...
  const { issueKey, issueSummary, dateISO, worklogs } = props;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <Button
          onClick={() => trackTime({ issueKey, date: dateISO })}
          variant="default"
        >
          Registrar tempo
        </Button>
              ...
              >
                <div className="text-sm">
                  <div className="font-medium">
                    {log.started
                      ? format(new Date(log.started), "dd/MM/yyyy HH:mm")
                      : "Sem data"}
                  </div>
                  <div className="text-muted-foreground">
                    {highlight ? "Neste dia" : "Outro dia"}
                  </div>
                </div>
                <div className="text-sm font-mono">
                  {(log.timeSpentSeconds ?? 0) / 3600 > 0
                    ? `${Math.floor((log.timeSpentSeconds ?? 0) / 3600)}h ${Math.round(
                        (((log.timeSpentSeconds ?? 0) % 3600) / 60) * 10
                      ) / 10}m`
...
export function IssueDayCell(props: IssueDayCellProps) {
  ...
  return (
    <TableCell
      onClick={openModal}
      className={cn(
        "cursor-pointer hover:bg-accent/40 transition-colors text-center",
        className
      )}
    >
      {displayValue}
