...
export async function WorklogStats({ targetHours = 160 }: Props) {
  ...
  const elapsedWorkingDays = workingDays.filter(
    (d) => !isAfter(d.date, today)
  ).length;
  ...
  const baselineDailyHours =
    totalWorkingDays > 0 ? targetHours / totalWorkingDays : 0;
  const requiredDailyHours =
    remainingWorkingDays > 0 ? remainingHours / remainingWorkingDays : 0;

  const monthProgressPct =
    totalWorkingDays > 0
      ? Math.min(
          100,
          Math.max(0, (elapsedWorkingDays / totalWorkingDays) * 100)
        )
      : 0;
  const hoursProgressPct =
    targetHours > 0
      ? Math.min(100, Math.max(0, (loggedHours / targetHours) * 100))
      : 0;
      ...
      <CardContent className="py-6">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <div className="space-y-1">
            ...
            <div className="text-xs text-muted-foreground">
              Faltam {remainingWorkingDays}
            </div>
          </div>
          <div className="space-y-1">
            <div className="text-sm text-muted-foreground">
              Ritmo necessário (restante)
            </div>
            <div className="text-2xl font-semibold">
              {formatHours(requiredDailyHours)}
            </div>
            <div className="text-xs text-muted-foreground">
              Média alvo: {formatHours(baselineDailyHours)}/dia
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-3">
          <div className="space-y-1">
            ...
            <div className="text-xl font-medium">
              {formatHours(expectedHoursByNow)}
            </div>
          </div>
          <div className="space-y-1">
            <div className="text-sm text-muted-foreground">
            ...
            <div
              className={`text-xl font-medium ${
                remainingHours > 0 ? "" : "text-muted-foreground"
              }`}
            >
