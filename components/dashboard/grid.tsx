    ...
    <>
            ...
            {items.map((item) => (
              <TableRow key={item.issue.id ?? item.issue.key}>
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <a
                          href={`${jiraBaseUrl}/browse/${item.issue.key}`}
                          target="_blank"
                ...
                {days.map(({ date, isWeekend }) => {
                  ...
                  const value =
                    seconds > 0 ? Duration.seconds(seconds).format() : "—";
                  const today = isToday(date);
                  return (
                    <IssueDayCell
                      key={`${
                        item.issue.id ?? item.issue.key
                      }-${date.toString()}`}
                      className={cn(
                        "text-center",
                        today
            ...
            <TableRow>
              ...
              {days.map(({ date, isWeekend }) => {
                const totalSeconds = items.reduce(
                  (acc, item) => acc + secondsForIssueOnDate(item, date),
                  0
                );
                const value =
                  totalSeconds > 0
                    ? Duration.seconds(totalSeconds).format()
                    : "—";
                const today = isToday(date);
                return (
                  <TableCell
      ...
      <div className="flex justify-between py-4">
        <div className="text-muted-foreground">
          Período: {format(new Date(data.from), "dd/MM/yyyy")} —{" "}
          {format(new Date(data.to), "dd/MM/yyyy")}
        </div>
        <div className="font-semibold">
          Total no período:{" "}
          {data.totalTimeSpent?.format?.() ?? String(data.totalTimeSpent)}
        </div>
      </div>
    </>
  );
