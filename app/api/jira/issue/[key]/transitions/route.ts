...
export async function GET(request: NextRequest, { params }: { params: Promise<{ key: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, transitions: data.transitions ?? [] });
  } catch (error) {
...
export async function POST(request: NextRequest, { params }: { params: { key: string } }) {
  try {
    ...
    const client = await jiraClientFactory();
    const jira = new Jira(client);
    await jira.transitionIssue(params.key, transitionId);

    return NextResponse.json({ success: true });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ message: "Dados inválidos", details: error.issues }, { status: 400 });
    }
